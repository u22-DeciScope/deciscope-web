...
export default function Meeting() {
  ...
  return (
    <div
      className="h-screen flex overflow-hidden p-[9px] gap-[8px]"
      style={{ background: "var(--ds-bg)" }}
    >
      {/* ============================================================
          LEFT: SideNav カード（フルハイト白カード） + Chat カードが重なる
          ============================================================ */}
      <div className="relative w-[294px] shrink-0 flex flex-col">

        {/* SideNav 背景カード */}
        <div className="absolute inset-0 bg-white rounded-[9px]" />

        {/* ── ロゴ行（上部 50px、カード全幅） ── */}
        <div className="relative z-10 flex items-center gap-2 h-[50px] pl-[9px] shrink-0">
          {/* ✦ アイコン */}
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-[22px] h-[22px] text-slate-700 shrink-0">
        ...
        <div className="relative flex-1 flex">
          ...
          {/* ナビストリップ */}
          <div className="relative z-10 w-[56px] flex flex-col items-center pt-3 pb-4 gap-4 shrink-0">
            {/* + ボタン */}
            <button
              type="button"
              className="w-[26px] h-[26px] rounded-full border-2 flex items-center justify-center transition hover:bg-slate-100"
          ...
          <div
            className="absolute left-[56px] top-0 right-[10px] bottom-[6px] flex flex-col overflow-hidden rounded-[14px]"
            style={{ background: "var(--ds-surface)", boxShadow: "var(--ds-shadow)" }}
          >
            {/* Chat ヘッダー */}
            <div
              className="h-[40px] flex items-center px-3 shrink-0 border-b"
              style={{ borderColor: "var(--node-border)" }}
            >
              <span className="w-[10px] h-[10px] rounded-full shrink-0" style={{ background: "var(--brand)" }} />
              <span className="ml-2 text-[12px] font-semibold" style={{ color: "var(--text-main)" }}>会話</span>
            </div>

            {/* メッセージ一覧 */}
            <div className="flex-1 overflow-y-auto px-2 py-2 space-y-[6px]">
              {chat.map((msg) =>
                msg.own ? (
                  /* 自分のメッセージ */
                  <div key={msg.id} className="flex justify-end pr-1">
                    <div
                      className="text-white text-[11px] rounded-[12px] px-[9px] py-[8px] max-w-[180px] leading-[16px] whitespace-pre-line"
                  ...
                  /* 相手のメッセージ */
                  <div key={msg.id}>
                    <p className="text-[10px] font-semibold pl-[8px] mb-[4px]" style={{ color: "var(--text-sub)" }}>
                      {msg.user}
                    </p>
                    <div
                      className="text-[11px] rounded-[12px] px-[8px] py-[7px] w-[180px] leading-[16px] border"
                      style={{
      ...
      <div className="flex-1 flex flex-col gap-[8px] min-w-0">
        ...
        <div
          className="h-[52px] bg-white rounded-[9px] border-2 border-red-500 flex items-center px-6 shrink-0"
        >
          ...
          <div className="flex items-center gap-2">
            <div className="w-[26px] h-[26px] rounded-full border-2 border-red-500 flex items-center justify-center">
        ...
        <div className="flex-1 flex gap-[8px] min-h-0">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-[6px]">
              {tree.map((node) => {
                ...
                return (
                  <div
                    key={node.id}
                    className="relative flex items-start gap-[6px] rounded-[10px] overflow-hidden border"
                    style={{
                  ...
                  >
                    ...
                    {node.indent > 0 && (
                      <div
                        className="absolute left-[-20px] top-0 bottom-0 w-[2px]"
                        style={{ background: "var(--indent-line)" }}
                      />
                    )}

                    <div className="flex items-start gap-[6px] px-[7px] py-[8px] w-full">
                      {/* タグバッジ */}
                      <span
                        className="shrink-0 text-[9px] font-semibold px-[5px] py-[3px] rounded-[4px] leading-none"
                        style={{ background: ts.bg, color: ts.fg }}
                      >
                        {node.tag}
                      </span>
                      {/* ユーザー名 */}
                      <span className="shrink-0 text-[10px] font-medium mt-[1px]" style={{ color: "var(--text-sub)" }}>
                        {node.user}
                      </span>
                      {/* 本文 */}
                      <span className="flex-1 text-[12px] leading-normal mt-[1px]" style={{ color: "var(--text-main)" }}>
                        {node.text}
                      </span>
                      {/* 時刻 */}
                      <span className="shrink-0 text-[10px] mt-[1px]" style={{ color: "var(--text-muted)" }}>
                        {node.time}
                      </span>
                    </div>
                  </div>
          ...
          {/* AI アシスタントカード */}
          <div
            className="w-[260px] shrink-0 flex flex-col overflow-hidden rounded-[14px]"
            style={{ background: "var(--ds-surface)", boxShadow: "var(--ds-shadow)" }}
          >
            {/* AIヘッダー */}
            <div
            ...
            >
              <div
                className="w-[22px] h-[22px] rounded-[6px] flex items-center justify-center text-white text-[12px] shrink-0"
              ...
              <div
                className="w-[18px] h-[18px] rounded-[9px] flex items-center justify-center text-white text-[10px] font-bold shrink-0"
                style={{ background: "var(--brand)" }}
              >
                3
              </div>
            ...
            {/* フィルタータブ */}
            <div
              className="h-[34px] flex items-center px-2 gap-1 shrink-0 border-b"
              style={{ borderColor: "var(--node-border)" }}
            >
              {["すべて", "⚠️ リスク", "💡 論点", "❓ 質問"].map((label, i) => (
                <button
                  key={label}
                  type="button"
                  className="text-[10px] px-[5px] py-[4px] rounded-[6px] whitespace-nowrap transition"
                  style={
                    i === 0
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 space-y-2">
              {insights.map((ins) => (
                <div key={ins.id}>
                  ...
                  <div
                    className="rounded-[10px] border p-[10px] relative"
                    style={{ background: ins.bg, borderColor: ins.border }}
                  >
                    {/* バッジ行 */}
                    <div className="flex items-center justify-between mb-[8px]">
                      <span
                        className="text-[9px] font-semibold px-[5px] py-[3px] rounded-[4px]"
                    ...
                    {/* タイトル */}
                    <p className="text-[12px] font-semibold leading-[17px] mb-[8px]" style={{ color: "var(--text-main)" }}>
                      {ins.title}
                    </p>
                    {/* 説明 */}
                    <p className="text-[11px] leading-[16px] mb-[10px]" style={{ color: "var(--text-sub)" }}>
                      {ins.desc}
                    </p>
                    {/* フッター */}
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-1">
                        <button
                          type="button"
                          className="w-[22px] h-[20px] rounded-[6px] flex items-center justify-center text-[11px] font-semibold"
                        ...
                        <button
                          type="button"
                          className="w-[22px] h-[20px] rounded-[6px] flex items-center justify-center text-[11px]"
